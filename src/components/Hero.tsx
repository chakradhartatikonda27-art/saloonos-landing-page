...
import { ArrowRight, Play, CheckCircle2, Sparkles, ShieldCheck, Zap, Scissors, HeartHandshake, Crown, Sparkle } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <div className="mt-6 flex flex-wrap items-center justify-center gap-2 max-w-3xl mx-auto">
          {BUSINESS_TYPES.map((b) => (
            <span 
              key={b.id}
              className="px-3 py-1 rounded-full bg-slate-900/80 border border-slate-800 text-slate-300 text-xs font-semibold hover:border-indigo-500/50 transition-colors"
            >
              ✨ {b.title}
            </span>
          ))}
