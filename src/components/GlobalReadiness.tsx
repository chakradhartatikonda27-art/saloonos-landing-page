...
export const GlobalReadiness: React.FC<GlobalReadinessProps> = ({ activeRegion, onRegionChange, onOpenDemo }) => {
  ...
  return (
    <section id="global-readiness" className="py-20 md:py-28 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-10 max-w-4xl mx-auto glass-card rounded-2xl p-6 md:p-8 border border-indigo-500/30 space-y-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
            <div className="flex items-center gap-3">
              <span className="text-3xl">{currentRegion.flag}</span>
              <div>
                <h3 className="text-2xl font-bold text-white font-heading">{currentRegion.country} Market Readiness</h3>
                <p className="text-xs text-indigo-300 font-semibold mt-0.5">{currentRegion.headline}</p>
              </div>
            </div>
            <span className="px-3 py-1 text-[11px] font-bold bg-indigo-500/20 text-indigo-300 rounded-full border border-indigo-500/30">
              LOCALIZED WORKFLOWS
            </span>
          </div>
          ...
          {/* Tax Compliance Note */}
          <div className="p-3.5 rounded-xl bg-slate-900/60 border border-slate-800 text-xs text-slate-400 flex items-center gap-2 font-mono">
            <ShieldCheck className="w-4 h-4 text-indigo-400 shrink-0" />
            <span>{currentRegion.taxComplianceNote}</span>
          </div>

          <div className="pt-2 flex justify-between items-center">
            <div className="text-xs text-slate-400">Available across all 190+ countries</div>
            <button
