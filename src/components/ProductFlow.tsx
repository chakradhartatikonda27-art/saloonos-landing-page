...
import { WORKFLOW_STAGES } from '../data/marketingData';
import { 
  Search, CalendarCheck, MapPin, Clock, Scissors, Receipt, CreditCard, RotateCw, TrendingUp
} from 'lucide-react';
...
export const ProductFlow: React.FC = () => {
  ...
  return (
    <section id="workflow" className="py-20 md:py-28 bg-[#080C14] theme-light:bg-slate-100/60 relative border-t border-slate-800 theme-light:border-slate-200 transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4">
          ...
          <p className="text-base sm:text-lg text-slate-300 theme-light:text-slate-600">
            See how Salon OS seamlessly connects every step of the client journey with your floor operations and business intelligence.
          </p>
        </div>
        ...
        <div className="mt-14 overflow-x-auto pb-6 scrollbar-thin">
          <div className="flex items-center justify-between min-w-[900px] relative px-4">
            ...
            <div className="absolute left-8 right-8 top-6 h-0.5 bg-slate-800 theme-light:bg-slate-300 -z-0" />
            <div 
                ...
                >
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-bold text-xs transition-all duration-300 ${
                    isActive
                      ...
                      : isPassed
                      ? 'bg-indigo-950 theme-light:bg-indigo-100 border border-indigo-500/50 text-indigo-400 theme-light:text-indigo-700'
                      : 'bg-slate-900 theme-light:bg-white border border-slate-800 theme-light:border-slate-300 text-slate-400 theme-light:text-slate-600 hover:border-indigo-400'
                  }`}>
                    <StageIcon className="w-5 h-5" />
                  </div>

                  <span className={`mt-3 text-xs font-bold font-heading tracking-wider uppercase transition-colors ${
                    isActive ? 'text-white theme-light:text-slate-900 font-extrabold' : 'text-slate-400 theme-light:text-slate-600 group-hover:text-slate-900'
                  }`}>
                    {stage.title}
                  </span>
                  
                  <span className="text-[10px] text-slate-400 theme-light:text-slate-500 font-mono mt-0.5">
                    {stage.step}
                  </span>
                </button>
              );
        ...
        <div className="mt-8 max-w-4xl mx-auto glass-card rounded-2xl p-6 md:p-8 border border-indigo-500/30 relative">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            ...
            <div className="flex items-start gap-4">
              <div className="w-14 h-14 rounded-2xl bg-indigo-600/20 text-indigo-500 flex items-center justify-center border border-indigo-500/40 shrink-0">
                <IconComp className="w-7 h-7" />
              </div>
              <div>
                <div className="flex items-center gap-3">
                  <span className="px-2.5 py-0.5 text-[10px] font-bold bg-indigo-500/20 text-indigo-600 theme-light:text-indigo-700 rounded uppercase">
                    STAGE {current.step} OF 09
                  </span>
                  <span className="text-xs text-slate-400 theme-light:text-slate-600 font-mono uppercase tracking-wider">
                    {current.title} PROCESS
                  </span>
                </div>
                <h3 className="text-2xl font-bold text-white theme-light:text-slate-900 font-heading mt-1">{current.title} Phase</h3>
                <p className="mt-2 text-sm text-slate-300 theme-light:text-slate-700 leading-relaxed font-normal">
                  {current.desc}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 shrink-0 self-end md:self-center">
              <button
                ...
                className="px-4 py-2 text-xs font-semibold rounded-lg bg-slate-900 theme-light:bg-white border border-slate-800 theme-light:border-slate-300 text-slate-300 theme-light:text-slate-700 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-slate-800"
              >
                ← Previous
              </button>
              <button
